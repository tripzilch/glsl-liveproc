...
vec3 trap(vec2 Z) {
    ...
    float L = length(X);
    vec3 c = mix(tex(X),vec3(1.0),step(.45,L));
    return c;
}
...
void main (void) {
    ...
    float i = 0.0;
    vec3 color = (i >= min_iter) ? trap(Z) : vec3(1.0);
    for (i = 1.; i < MAXITER; i++) {
        if (Zmag2 < BAILOUT2) {
            ...
            if (i >= min_iter) {
                vec3 t = trap(Z);
                //grey = min(max(grey, gg), 1.0 - min(grey, gg));
                color *= t;
            }
        } else { i = MAXITER; }
    }
    ...
    // gamma, output
    // gl_FragColor = vec4(vec3(pow(grey, gamma.x)), alpha);
