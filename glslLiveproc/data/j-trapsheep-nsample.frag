...
vec3 background_color = vec3(0.05, 0.1, 0.02); // tex(vec2(-0.5,-0.5));
vec3 texex(vec2 X) {
    float L = length(X);
    ...
    for (uint n = 0u; n < uint(N_SAMPLES); n++) {
        ...
        for (i = 1.; i < MAXITER; i++) {
            if (Zmag2 < BAILOUT2) {
                ...
                Zmag2 = Z2.x + Z2.y;
                //c *= texplex(Z, dZ, i);
                vec3 d = texplex(mix(pZ, Z, .5), dZ, i);
                float cg = smoothstep(0.0, 1.0, dot(c, vec3(-0.3, 1.0, -0.3)));
                float dg = smoothstep(0.0, 1.0, dot(d, vec3(-0.3, 1.0, -0.3)));
                c = mix(c, d, smoothstep(dg * .8, dg * .9, cg));
            }
        }
